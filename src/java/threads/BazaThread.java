 ...
 * @author MinhTran
 */
public class BazaThread extends BaseThread implements Runnable {

    private ServletContext context;
    private final String configPath = "WEB-INF/targets/bazavn.xml";

    public BazaThread(ServletContext context) {
            ...
            try {
                for (int i = 0; i < nodeList.getLength(); i++) {
                    ...
                    if (node.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        Thread thread = new Thread(new BazaCrawler(context, url, sex, categoryId));
                        thread.start();
                    }
                }
            } catch (NumberFormatException e) {
                Logger.getLogger(BazaThread.class.getName()).log(Level.SEVERE, null, e);
            }
